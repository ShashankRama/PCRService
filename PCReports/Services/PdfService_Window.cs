...
using PCReports.Models.Window;

namespace PCReports.Services.Window
{
...

    {
        ...
        public string GenerateReport(string pdfFilePath, PDFResult pdfResult)
        {
            ...
            string source1 = _resourceFolderPath + @"templates\SPS Structural report tempelate_Part1 - EN.pdf";
            string source2 = _resourceFolderPath + @"templates\SPS Structural report tempelate_Part2 - EN.pdf";
            string source3 = _resourceFolderPath + @"templates\SPS Structural report tempelate_Part3 - EN.pdf";
            if (Thread.CurrentThread.CurrentCulture.Name == "de-DE")
            {
                source1 = _resourceFolderPath + @"templates\SPS Structural report tempelate_Part1 - DE.pdf";
                source2 = _resourceFolderPath + @"templates\SPS Structural report tempelate_Part2 - DE.pdf";
                source3 = _resourceFolderPath + @"templates\SPS Structural report tempelate_Part3 - DE.pdf";
            }
            if (Thread.CurrentThread.CurrentCulture.Name == "fr-FR")
            {
                source1 = _resourceFolderPath + @"templates\SPS Structural report tempelate_Part1 - FR.pdf";
                source2 = _resourceFolderPath + @"templates\SPS Structural report tempelate_Part2 - FR.pdf";
                source3 = _resourceFolderPath + @"templates\SPS Structural report tempelate_Part3 - FR.pdf";
            }
        ...
        public void PdfInputMemberData(PDFResult pdfResult, int subSectionNo, PdfDocument myTemplate)
        {
            ...
            InsertValue(myTemplate, "TributaryArea", $"{pdfMemberResult.TributaryArea,3:0.0}");
        ...
        public void PdfDrawLoadCaseImageTitle(PdfDocument myTemplate, bool isTransom, int chartNo, double yref)
        {
            ...
            XBrush graybrush = new XSolidBrush(XColor.FromArgb(102, 102, 102));
            XBrush brush = (chartNo == 4 || chartNo == 5) ? graybrush : whitebrush;
        ...
        public void PdfDrawLoads(PDFMemberResult pdfMemberResult, PdfDocument myTemplate, int chartNo, double yref)
        {
            ...
            if (dimNumber == 0)
            {
                PdfDrawDimension(myTemplate.Pages[0], 0, dimData[dimData.Length - 1], 0, dimData[dimData.Length - 1] * len * 10, xLocation, yLocation, Length);
            }
            else if (chartNo == 1 || chartNo == 2)
            {
                PdfDrawDimension(myTemplate.Pages[0], 0, dimData[dimData.Length - 1], 1, dimData[dimData.Length - 1] * len * 10, xLocation, yLocation, Length);
            }
        }
        ...
        internal void MergeLoadCaseData(double[,] loadCaseData)  // merge concentrated loads at same location
        {
            ...
            {
                if (Convert.ToInt32(loadCaseData[i, 8]) != 2 && Convert.ToInt32(loadCaseData[i, 8]) != 4) continue;
        ...
        public void PdfDrawReactions(PDFMemberResult pdfMemberResult, PdfPage myPage, int chartNo, double yref)
        {
            ...
            {
                double[] reactionForce = pdfMemberResult.VerticalReactionForce;
                // draw reactions
                double xLocation = 100;
                double yLocation = chartNo == 5 ? yref + 18 : yref + 31;
                double beamLength = 400;
                ...
                // draw the left reaction
                if (Math.Abs(reactionForce[0]) > 0.0001)
                {
                    x = xLocation;
                    y = yLocation;
                    PdfDrawVerticalReaction(myPage, x, y, reactionForce[0], 0, "left");
                }

                // draw the right reaction
                if (Math.Abs(reactionForce[1]) > 0.0001)
                {
                    x = xLocation + beamLength;
                    y = yLocation;
                    PdfDrawVerticalReaction(myPage, x, y, reactionForce[1], 0, "right");
                }
            }
        }
        ...
        private double CalculateLimit(double yMax)
        {
            ...
            if (yMax < 1)
            {
                yLimit = Math.Max(Math.Ceiling(yMax / 0.2) * 0.2, 0.2);
            }
            else if (yMax < 3)
            {
                yLimit = Math.Max(Math.Ceiling(yMax / 1) * 1, 1);
            }
            else if (yMax < 30)
            {
